...
public class WxXmlUtil {
    ...
    public static WxMsgEventEntity getMsgEvent(Element ele) throws DocumentException {
         ...
         * 上报地理位置事件
         * 
         * 用户同意上报地理位置后，每次进入公众号会话时，都会在进入时上报地理位置，或在进入会话后每5秒上报一次地理位置，公众号可以在公众平台网站中
         * 修改以上设置。上报地理位置时，微信会将上报地理位置事件推送到开发者填写的URL。
         */
        if (ele.elementText("Latitude") != null) {
            result.setLatitude(doubleVal(ele, "Latitude"));
        }
        if (ele.elementText("Longitude") != null) {
            result.setLatitude(doubleVal(ele, "Longitude"));
        }
        if (ele.elementText("Precision") != null) {
            result.setLatitude(doubleVal(ele, "Precision"));
        }
         ...
         */
        if (ele.elementText("Status") != null) {
            result.setStatus(strVal(ele, "Status"));
        }
        if (ele.elementText("TotalCount") != null) {
            result.setTotalCount(intVal(ele, "TotalCount"));
        }
        if (ele.elementText("FilterCount") != null) {
            result.setTotalCount(intVal(ele, "FilterCount"));
        }
        if (ele.elementText("SentCount") != null) {
            result.setTotalCount(intVal(ele, "SentCount"));
        }
        if (ele.elementText("ErrorCount") != null) {
            result.setTotalCount(intVal(ele, "ErrorCount"));
        }
